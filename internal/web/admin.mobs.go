...
func mobsIndex(w http.ResponseWriter, r *http.Request) {
	...
	tmpl, err := template.New("index.html").Funcs(funcMap).ParseFiles(configs.GetConfig().FolderAdminHtml.String()+"/_header.html", configs.GetConfig().FolderAdminHtml.String()+"/mobs/index.html", configs.GetConfig().FolderAdminHtml.String()+"/_footer.html")
	if err != nil {
...
func mobData(w http.ResponseWriter, r *http.Request) {
	...
	tmpl, err := template.New("mob.data.html").Funcs(funcMap).ParseFiles(configs.GetConfig().FolderAdminHtml.String() + "/mobs/mob.data.html")
	if err != nil {
		mudlog.Error("HTML Template", "error", err)
	}
