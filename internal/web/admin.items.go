...
func itemsIndex(w http.ResponseWriter, r *http.Request) {
	...
	tmpl, err := template.New("index.html").Funcs(funcMap).ParseFiles(configs.GetFilePathsConfig().FolderAdminHtml.String()+"/_header.html", configs.GetFilePathsConfig().FolderAdminHtml.String()+"/items/index.html", configs.GetFilePathsConfig().FolderAdminHtml.String()+"/_footer.html")
	if err != nil {
...
func itemData(w http.ResponseWriter, r *http.Request) {
	...
	tmpl, err := template.New("item.data.html").Funcs(funcMap).ParseFiles(configs.GetFilePathsConfig().FolderAdminHtml.String() + "/items/item.data.html")
	if err != nil {
		mudlog.Error("HTML Template", "error", err)
	}
