...
func TelnetIACHandler(clientInput *connections.ClientInput, sharedState map[string]any) (nextHandler bool) {
	...
	//slog.Debug("Received", "type", "IAC (TEST)", "data", term.BytesString(clientInput.DataIn))
	...
	for _, iacCmd := range iacCmds {
		...
		if term.IsGMCPCommand(iacCmd) {
			...
			if ok, payload := term.Matches(iacCmd, term.GmcpAccept); ok {
				slog.Debug("Received", "type", "IAC (Client-GMCP Accept)", "data", term.BytesString(payload))
				continue
			}

			if ok, payload := term.Matches(iacCmd, term.GmcpRefuse); ok {
				slog.Debug("Received", "type", "IAC (Client-GMCP Refuse)", "data", term.BytesString(payload))
				continue
			}
			...
			if len(iacCmd) >= 5 && iacCmd[len(iacCmd)-2] == term.TELNET_IAC && iacCmd[len(iacCmd)-1] == term.TELNET_SE {
				...
				//slog.Debug("Received", "type", "GMCP", "size", len(iacCmd), "data", string(requestBody))
				...
				case `Char.Login`:
					...
					if err := json.Unmarshal(payload, &decoded); err == nil {
						slog.Debug("GMCP LOGIN", "username", decoded.Name, "password", decoded.Password)
					}
				}
			...
			// Unhanlded IAC command, log it
			slog.Debug("Received", "type", "GMCP?", "size", len(iacCmd), "data", string(iacCmd))
		...
		if term.IsMSPCommand(iacCmd) {
			...
			if ok, payload := term.Matches(iacCmd, term.MspAccept); ok {
				slog.Debug("Received", "type", "IAC (Client-MSP Accept)", "data", term.BytesString(payload))
			...
			if ok, payload := term.Matches(iacCmd, term.MspRefuse); ok {
				slog.Debug("Received", "type", "IAC (Client-MSP Refuse)", "data", term.BytesString(payload))
		...
		if ok, payload := term.Matches(iacCmd, term.TelnetAcceptedChangeCharset); ok {
			slog.Debug("Received", "type", "IAC (TelnetAcceptedChangeCharset)", "data", term.BytesString(payload))
			continue
		}

		if ok, _ := term.Matches(iacCmd, term.TelnetRejectedChangeCharset); ok {
			slog.Debug("Received", "type", "IAC (TelnetRejectedChangeCharset)")
			continue
		}

		if ok, _ := term.Matches(iacCmd, term.TelnetAgreeChangeCharset); ok {
			slog.Debug("Received", "type", "IAC (TelnetAgreeChangeCharset)")
			connections.SendTo(
		...
		if ok, _ := term.Matches(iacCmd, term.TelnetDontSuppressGoAhead); ok {
			slog.Debug("Received", "type", "IAC (TelnetDontSuppressGoAhead)")
		...
		if ok, payload := term.Matches(iacCmd, term.TelnetScreenSizeResponse); ok {
			...
			if err != nil {
				slog.Debug("Received", "type", "IAC (Screensize)", "data", term.BytesString(payload), "error", err)
			} else {
				slog.Debug("Received", "type", "IAC (Screensize)", "width", w, "height", h)
		...
		// Unhanlded IAC command, log it
		slog.Debug("Received", "type", "IAC (Unhandled)", "size", len(clientInput.DataIn), "data", term.TelnetCommandToString(iacCmd))
