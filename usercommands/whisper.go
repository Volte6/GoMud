...
func Whisper(rest string, userId int, cmdQueue util.CommandQueue) (util.MessageQueue, error) {
	...
	if toUser == nil {
		response.SendUserMessage(userId, "You can't find anyone by that name.", true)
		response.Handled = true
		return response, nil
	}
