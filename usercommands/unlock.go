...
func Unlock(rest string, userId int, cmdQueue util.CommandQueue) (util.MessageQueue, error) {
	...
	if containerName != `` {
		...
		if !container.Lock.IsLocked() {
			response.SendUserMessage(userId, "That's not locked.", true)
			response.Handled = true
			return response, nil
		}
		...
		if hasKey {
			...
			response.SendUserMessage(userId, fmt.Sprintf(`You use a key to unlock the <ansi fg="container">%s</ansi>.`, containerName), true)
			response.SendRoomMessage(user.Character.RoomId, fmt.Sprintf(`<ansi fg="username">%s</ansi> uses a key to unlock the <ansi fg="container">%s</ansi>.`, user.Character.Name, containerName), true)
			...
			response.SendUserMessage(userId, `You do not have the key for that. Maybe you could <ansi fg="command">picklock</ansi> the lock.`, true)
		}
		...
		if hasKey {
			...
			response.SendUserMessage(userId, fmt.Sprintf(`You use a key to unlock the <ansi fg="exit">%s</ansi> lock.`, exitName), true)
			response.SendRoomMessage(user.Character.RoomId, fmt.Sprintf(`<ansi fg="username">%s</ansi> uses a key to unlock the <ansi fg="exit">%s</ansi> lock`, user.Character.Name, exitName), true)
		} else if hasBackpackKey {
