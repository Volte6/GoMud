...
func IfItem(rest string, mobId int, cmdQueue util.CommandQueue) (util.MessageQueue, error) {
	...
	for _, uid := range room.GetPlayers() {
		if user := users.GetByUserId(uid); user != nil {
			...
			_, found := user.Character.FindInBackpack(itemName)
			if !found {
				_, found = user.Character.FindOnBody(itemName)
			}
...
// if -1-start Say has anyone seen my locket?
// The second message will only be executed if the quest is successfully given.
func IfNotItem(rest string, mobId int, cmdQueue util.CommandQueue) (util.MessageQueue, error) {
