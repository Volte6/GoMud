...
function onCommand_look(rest, user, room) {
    ...
    SendRoomMessage(room.RoomId(), "<ansi fg=\"username\">"+user.GetCharacterName()+"</ansi> examines an old lantern on the wall.");   
...
function onCommand(cmd, rest, user, room) {
    ...
    SendRoomMessage(room.RoomId(), "<ansi fg=\"username\">"+user.GetCharacterName()+"</ansi> does something with the lantern, and a secret passage opens! He disappears through the passage just as it closes.");
    user.MoveRoom(50);
